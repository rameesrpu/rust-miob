...
*
******************************************************************************/
int fpgatimer1s(int ledblinkcount)
{
	...
	while (1) {
		 ...
		 */
		while (exit_flag < ledblinkcount)
		{
			if(1== XGpioPs_ReadPin(&PsGpio,OUTPUT_PIN))
			{
				print("PS LED Turned OFF \r\n");
				XGpioPs_WritePin(&PsGpio,OUTPUT_PIN,0);
			}
			else
			{
				print("PS LED Turned ON \r\n");
